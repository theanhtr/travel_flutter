...
...Đã làm chuyển ngữ
*/
class UpdateTypeRoomScreen extends StatefulWidget {
  const UpdateTypeRoomScreen({super.key});

  static const String routeName = '/update_type_room_screen';

  @override
  State<UpdateTypeRoomScreen> createState() => _UpdateTypeRoomScreenState();
}

class _UpdateTypeRoomScreenState extends State<UpdateTypeRoomScreen> {
  String roomName = "";
  String description = "";
  String price = "";
  String occupancy = "";
  ...
  UpdateTypeRoomController? _controller;

  @override
  Widget build(BuildContext context) {
    _controller = UpdateTypeRoomController();
    final Map<String, dynamic> args = NavigationUtils.getArguments(context);
    final RoomModel roomSelect = args['room_selected'];
    roomName = roomSelect.name!;
    description = roomSelect.description!;
    price = roomSelect.price.toString();
    occupancy = roomSelect.occupancy.toString();
    numberOfBeds = roomSelect.numberOfBeds.toString();
    roomSize = roomSelect.size.toString();
    amenitiesResults = roomSelect.amenitiesResults!;
    for (var i = 0; i < amenitiesResults.length; i++) {
      if (amenities == "") {
        amenities += '${amenitiesResults[i] + 1}';
      } else {
        amenities += ',${amenitiesResults[i] + 1}';
      }
    }
    return AppBarContainer(
      titleString: LocalizationText.updateTypeRoom,
          ...
          children: [
            ...
            ButtonWidget(
              ...
              ontap: () {
                ...
                _controller
                    ?.updateTypeRoom(roomSelect.id!, roomName, description,
                        price, occupancy, numberOfBeds, roomSize, amenities)
                    .then((value) => {
