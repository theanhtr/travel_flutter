...
class MyhotelScreen extends StatefulWidget {
  ...
  Function setSectiond;
  MyhotelScreen(
      {super.key, required this.hotelsList, required this.setSectiond});
...
class _MyhotelScreenState extends State<MyhotelScreen> {
      ...
      await _controller?.getDistanceInformation(element.address ?? "").then(
          ...
          onError: (error) => {
                listHotelCardWidget.add(HotelCardWidget(
                  ...
                  priceInfo: element.priceInfo ?? "",
                  ontap: () async {
                    debugPrint("dong 101: ${widget.setSectiond}");
                    await NavigationUtils.navigate(
                        context, HotelDetailScreen.routeName,
                        arguments: {
                          "hotelId": element.id,
                          ...
                          'guestCount': 1,
                          'roomCount': 1,
                          'setSectiond': widget.setSectiond,
                        });
                  },
                )),
              });
  ...
  Widget build(BuildContext context) {
    ...
    if (isFirst) {
      ...
      print('lengthfffffffffffff dong 122 ${widget.hotelsList.length}');
      if (!_isLoading && !_loadHotelListDone) {
        // print('dang chạy loading đâyyyyyy');
        _isLoading = true;
        listHotel = [];
        print('lengthfffffffffffff dong 126 ${hotels.length}');
        List<dynamic> images;
        String imagePath = "";
        String address;
        ...
        hotels.forEach((element) async {
          ...
          await _controller?.getHotelDetail(element["id"]).then((value) => {
                ...
                _hotelOwnerManager.myHotelModel = hotel,
                if (listHotel.length < widget.hotelsList.length)
                  {
                    listHotel.add(hotel),
                  },
    ...
    if (widget.hotelsList.isEmpty == true) {
      print("dong 195");
      print(widget.hotelsList.isEmpty);
    }
    return SingleChildScrollView(
      child: widget.hotelsList.isEmpty == true
          ? Container(
